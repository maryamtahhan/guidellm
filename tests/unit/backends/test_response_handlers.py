...
class TestEmbeddingsResponseHandler:
    ...
    def test_non_streaming(
        ...
        assert result.input_metrics.text_tokens == expected_input_tokens
        assert result.output_metrics.text_tokens == 0  # Embeddings don't generate tokens
        ...
        [
            (
                [
                    ...
                    'data: {"data": [{"embedding": [0.4, 0.5, 0.6]}], "usage": {"prompt_tokens": 10}}',
                    "data: [DONE]",
                ],
                2,
                10,
    ...
    def test_streaming(
        ...
        assert len(text_data["embeddings"]) == expected_embeddings_count
        assert response.input_metrics.text_tokens == expected_input_tokens
        assert response.output_metrics.text_tokens == 0  # Embeddings don't generate tokens
